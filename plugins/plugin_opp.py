...
class GhostCQPlus:
    ...
    def _add_ghost_S(self, obj_id: int, size: int) -> None:
        ...
        while self.ghost_S_size > self.cache_size and self.ghost_S:
            oldest_id, oldest_size = self.ghost_S.popitem(last=False)
    ...
    def _add_ghost_M(self, obj_id: int, size: int) -> None:
        ...
        while self.ghost_M_size > self.cache_size and self.ghost_M:
            oldest_id, oldest_size = self.ghost_M.popitem(last=False)
    ...
    def on_miss(self, req: Request) -> None:
        ...
        # Gentle adaptation: reduced from 1.0% to 0.2% to stabilize large caches
        delta = max(obj_size, int(self.cache_size * 0.002))
        is_ghost_hit = False

        if obj_id in self.ghost_S:
            # Expanded max bound to 50% for highly scanned environments
        ...
        # CRITICAL FIX: If it's a known ghost, it has proven itself. Bypass probation.
        if is_ghost_hit:
    ...
    def pick_victim(self, req: Request) -> int:
        ...
        while True:
            ...
            self._add_ghost_M(evict_node.id, evict_node.size)
            return evict_node.id
