...
class GhostCQPlus:
    def __init__(self, cache_size: int):
        ...
        self.p_min_bytes = max(1, int(self.cache_size * 0.05))
        self.p_max_bytes = max(self.p_min_bytes, int(self.cache_size * 0.40))
    ...
    def on_hit(self, req: Request) -> None:
        ...
        if node is not None:
            # Keep very hot protected items near MRU while leaving one-hit items clock-like.
            if node.q_type == "protected" and node.visited and node is not self.m_head.next:
                prev_node = node.prev
                if self.hand == node:
                    self.hand = prev_node
                    if self.hand == self.m_head:
    ...
    def on_miss(self, req: Request) -> None:
        ...
        if obj_id in self.ghost_S:
            self.p_target_bytes = min(self.p_max_bytes, self.p_target_bytes + delta)
            self.ghost_S_size -= self.ghost_S[obj_id]
            del self.ghost_S[obj_id]
            from_ghost_s = True
        elif obj_id in self.ghost_M:
            self.p_target_bytes = max(self.p_min_bytes, self.p_target_bytes - delta)
            self.ghost_M_size -= self.ghost_M[obj_id]
            del self.ghost_M[obj_id]
            from_ghost_m = True

        # Admission:
        # - ghost-M: direct protected
        # - ghost-S: direct protected only under probation pressure, else warm probation
        # - new: cold probation
        node = Node(obj_id, obj_size)
        if from_ghost_m or (from_ghost_s and self.p_size_bytes > self.p_target_bytes + obj_size):
