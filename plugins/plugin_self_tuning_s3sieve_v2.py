...
class PolicyConfig:
    def __init__(
        self,
        probation_frac_arms=(0.16, 0.24, 0.32, 0.40, 0.50, 0.60),
        adapt_every: int = 1024,
        short_gap_epochs: int = 1,
        ghost_factor: int = 6,
...
class SelfTuningS3SieveV2:
    """
    Tuned online S3-SIEVE hybrid:
    - Probation + protected + ghost.
    - UCB bandit tunes probation-size arm online.
    - Faster adaptation window than v1.
    - Less conservative promotion outside scan mode.
    """
    ...
    def __init__(self, cache_size: int, cfg: Optional[PolicyConfig] = None):
        ...
        self.target_step = max(1, self.cache_size // 48)
    ...
    def _adapt_bandit(self) -> None:
        # Reward favors stable protected hits, penalizes churn/ghost pressure.
        reward = (
            2.2 * self.win_hit_protected
            + 0.6 * self.win_hit_probation
            - 1.8 * self.win_miss
            - 1.6 * self.win_ghost_from_probation
            - 0.6 * self.win_ghost_from_protected
    ...
    def on_hit(self, req: Request) -> None:
        ...
        if rec is not None:
            ...
            # Tuned promotion:
            # - in non-scan phase, quick second touch promotes
            # - in scan phase, require stronger evidence (ghost or prior warm bit)
            promote = False
            if short_gap:
    ...
    def pick_victim(self, req: Request) -> int:
        ...
        if self.protected and self.protected_bytes > (protected_target + 2 * self.target_step):
...
def init_hook(common_cache_params: CommonCacheParams) -> SelfTuningS3SieveV2:
    return SelfTuningS3SieveV2(cache_size=int(common_cache_params.cache_size))


def hit_hook(data: SelfTuningS3SieveV2, req: Request) -> None:
    data.on_hit(req)


def miss_hook(data: SelfTuningS3SieveV2, req: Request) -> None:
    data.on_miss(req)


def eviction_hook(data: SelfTuningS3SieveV2, req: Request) -> int:
    return data.pick_victim(req)


def remove_hook(data: SelfTuningS3SieveV2, obj_id: int) -> None:
    data.on_remove(obj_id)


def free_hook(data: SelfTuningS3SieveV2) -> None:
    data.on_free()
