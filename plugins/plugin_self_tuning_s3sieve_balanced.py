...
class PolicyConfig:
    def __init__(
        self,
        probation_frac_arms=(0.12, 0.18, 0.24, 0.30, 0.36, 0.42),
...
class SelfTuningS3SieveBalanced:
    """
    Balanced online S3-SIEVE hybrid:
    - Keeps the same probation/protected/ghost skeleton.
    - Uses milder probation-size arms and slower adaptation.
    - Promotes reuse sooner to avoid large-cache regressions.
    """
    ...
    def _adapt_bandit(self) -> None:
        # Reward tuned to avoid over-prioritizing scan filtering.
        reward = (
            2.4 * self.win_hit_protected
            + 0.9 * self.win_hit_probation
            - 1.4 * self.win_miss
            - 1.2 * self.win_ghost_from_probation
            - 0.5 * self.win_ghost_from_protected
    ...
    def on_hit(self, req: Request) -> None:
        ...
        if rec is not None:
            ...
            # More reuse-friendly than v1:
            # quick second touch promotes unless clearly scan-heavy and weak evidence.
            promote = False
            if short_gap:
                if not self.scan_mode:
                    promote = True
                elif ghost_mark == 1 or refbit == 1:
                    promote = True
    ...
    def _evict_from_probation(self) -> Optional[int]:
        ...
        for _ in range(n):
            ...
            # If protected still has room, convert warm probation survivors to protected.
            if self.protected_bytes < protected_target:
    ...
    def pick_victim(self, req: Request) -> int:
        ...
        # Only pressure protected when it is clearly over target.
        if self.protected and self.protected_bytes > (protected_target + 4 * self.target_step):
...
def init_hook(common_cache_params: CommonCacheParams) -> SelfTuningS3SieveBalanced:
    return SelfTuningS3SieveBalanced(cache_size=int(common_cache_params.cache_size))


def hit_hook(data: SelfTuningS3SieveBalanced, req: Request) -> None:
    data.on_hit(req)


def miss_hook(data: SelfTuningS3SieveBalanced, req: Request) -> None:
    data.on_miss(req)


def eviction_hook(data: SelfTuningS3SieveBalanced, req: Request) -> int:
    return data.pick_victim(req)


def remove_hook(data: SelfTuningS3SieveBalanced, obj_id: int) -> None:
    data.on_remove(obj_id)


def free_hook(data: SelfTuningS3SieveBalanced) -> None:
    data.on_free()
